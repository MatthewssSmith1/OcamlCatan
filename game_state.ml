...
let get_player state color =
  let rec helper list color =
    ...
    | h :: t -> if Player.get_color h = color then h else helper t color in
  ...
  let replace_player state color player =
    let rec helper list color player =
      match list with
      | [] -> []
      | h :: t -> if Player.get_color h = color then player :: t
      else helper t color player in
      {state with players = helper state.players color player}

  let give_resource state color resource amount =
    let new_player = Player.add_resource resource amount (get_player state color) in
  ...
  let rec vert_helper state verts resource =
    ...
    | Types.City color :: t ->
      vert_helper (give_resource state color resource 2) t resource in
